...
export default function ReturnPolicy() {
  ...
  const { data: policy } = useQuery<ReturnPolicyData>({
    queryKey: ["return-policy"],
    queryFn: async () => {
      const res = await apiClient.get("/settings/return-policy");
      return res.data;
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: Partial<ReturnPolicyData>) => {
      const res = await apiClient.put("/settings/return-policy", data);
      return res.data;
    },
    onSuccess: (data) => {
      queryClient.setQueryData(["return-policy"], data);
    },
  });

  const handleSectionUpdate = (
    index: number,
    field: keyof ReturnSection,
    value: string,
  ) => {
    if (!policy) return;
  ...
  const effectiveDate = policy?.effectiveDate || "20 ნოემბერი, 2025";
  const effectiveDateEn = policy?.effectiveDateEn || "November 20, 2025";

  return (
    <div className="return-container">
      <div className="return-content">
        <h1 className="return-title">
          {language === "en" ? "Return Policy" : "დაბრუნების პოლიტიკა"}
        </h1>
        ...
        {sections.map((section, idx) => {
          ...
          return (
            <div className="return-section" key={idx}>

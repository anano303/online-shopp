...
export function generateProductSchema(product: Product, productId: string) {
  const baseUrl =
    process.env.NEXT_PUBLIC_PRODUCTION_URL || "https://13.online.ge";
  ...
  const categoryName =
    typeof product.mainCategory === "object"
      ...
      : product.mainCategory || "მამაკაცების ტანსაცმელი";
  ...
  return {
    ...
    image: product.images?.length > 0 ? product.images : ["/api/logo"],
    sku: product._id,
    offers: {
      "@type": "Offer",
